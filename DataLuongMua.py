...
def create_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS luong_mua (
...
def insert_data(data):
    ...
    for entry in data:
        ...
        cursor.execute('''
            INSERT INTO luong_mua (
                mahieu, ten, kinhdo, vido, datetime, luongmua, thoigian
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            entry['mahieu'], entry['ten'], entry['kinhdo'], entry['vido'],
            entry['datetime'], entry['luongmua'], entry['thoigian']
        ))
...
# Main execution
api_url = 'http://qndnapi.vndss.com/service/getDataLuongMua'
